...
def score_one_task_dir(task_dir: str) -> Optional[TaskScore]:
    ...
    if isinstance(merged, list):
        ...
        for it in merged:
            ...
            kind = str(it.get("category") or "unknown")
        ...
        any_items = False
        for kind in ("Executability", "Interactivity", "Aesthetics"):
    ...
    if not found_any:
        ...
        for fname in checklist_files_to_scan:
            ...
            if data is None:
                continue
            kind = CHECKLIST_KIND_OF_FILE.get(fname, os.path.splitext(fname)[0])
...
def main() -> int:
    ...
    print(f"hard   (<=1/3): {len(buckets['hard'])}\t" + ",".join(buckets["hard"]))
    ...
    for k in ("Executability", "Interactivity", "Aesthetics"):
        vals = [s.harmonic_mean_by_kind.get(k) for s in scores_sorted]
    ...
    for g in ("Aesthetics", "Executability", "Interactivity"):
    ...
    if int(args.select) > 0:
        ...
        for row in selected_rows:
            ...
            if isinstance(kt, dict):
                for k, v in kt.items():
                    selected_kind_total[k] = selected_kind_total.get(k, 0) + int(v)
            if isinstance(kp, dict):
                for k, v in kp.items():
                    selected_kind_passed[k] = selected_kind_passed.get(k, 0) + int(v)
...
#     python3 /share/leixinping/Test/score_tiaohe.py \
#   --root /share/leixinping/Test/Bench_result_opus/Qwen3-VL-235B-A22B-Instruct/resume_site \
#   --select 0 \
#   --task-ids "7,15,26,84,115,124,191,199,239,243,255,262,288,347,354,428,440,455,465,469,474,478,480,486,541,545,549,550,552,554,558,569,572,580,617,630,648,668,699,780,795,801,804,817,832,850,852,868,901,908,932,953,956,89,363,70,137,253,470,601"

#     python3 /share/leixinping/Test/score_tiaohe.py \
#   --root /share/leixinping/Test/Bench_result_opus/Qwen3-VL-235B-A22B-Instruct/resume_site \
#   --select 0 \
#   --task-ids "6,7,15,26,33,47,58,70,84,88,89,100,108,115,122,124,125,137,156,191,199,213,226,227,230,231,235,238,239,243,248,253,255,262,270,273,276,287,288,291,293,299,312,318,332,335,347,351,354,363,428,440,443,446,453,455,460,462,463,465,469,470,474,478,480,486,490,491,505,519,541,545,547,548,549,550,552,554,555,558,563,565,567,569,572,580,601,609,613,617,628,630,648,657,664,668,679,694,698,699,707,738,743,754,779,780,785,795,801,804,817,832,850,852,868,883,901,907,908,909,932,953,956"

#     python3 /share/leixinping/Test/score_tiaohe.py \
#   --root /share/leixinping/Test/video_bench/Claude-4.5-Sonnet/resume_site \
#   --select 0 \
#   --task-ids "86,18,8,7,6,69,17,5,81,30,91,92,55,85,36,62,33,53,74,93,77,83,16,88,67,61,73,75,39,11,63,90,35,65,59,27,44,64,52,89,51,45,4,94,70,58,19,12,24,56,78,2,79,10,34,71,20,60,84,1,3,47,87,72,68,22,37,40,29,54,48,14,76,66,80,9,23,31,57,38,41,28,42,26,13,46,82,50,15,21,25,32,43,49"

#     python3 /share/leixinping/Test/score_tiaohe.py \
#   --root /share/leixinping/Test/video_bench/Qwen3-VL-235B-A22B-Instruct/resume_site \
#   --select 0 \
#   --task-ids "86,18,8,7,6,69,17,5,81,30,91,92,55,85,36,62,33,53,74,93,77,83,16,88,67,61,73,75,39,11,63,90,35,65,59,27,44,64,52,89,51,45,4,94,70,58,19,12,24,56,78,2,79,10,34,71,20,60,84,1,3,47,87,72,68,22,37,40,29,54,48,14,76,66,80,9,23,31,57,38,41,28,42,26,13,46,82,50,15,21,25,32,43,49"

# find /share/leixinping/Test/image_bench_v1/Gemini-3-Pro/resume_site -name "行消失完成.png"

#     python3 /share/leixinping/Test/score_tiaohe.py \
#   --root /share/leixinping/Test/Bench_result_opus/GPT-5.2/resume_site \
#   --select 0 \
#   --task-ids "15, 47, 70, 137, 191, 238, 243, 253, 262, 270, 273, 288, 291, 299, 318, 351, 354, 462, 463, 470, 480, 490, 491, 505, 547, 554, 569, 617, 679, 694, 699, 707, 738, 743, 754, 780, 795, 804, 817, 850, 868, 953"
