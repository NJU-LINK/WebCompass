#!/usr/bin/env python3
# example: python -m generation.evaluation.test_image
...
VERIFY_DESCRIPTION = """
<pr_description>
You are a strict website tester. Your task is to rigorously evaluate webpage execution, aesthetics, and interactive functionality based on a checklist (verification and scoring only, no fixes).
...
3) View reference images under screenshots/
...
2) For each item verification: execute user operations as required, observe if expectations are met. For items requiring comparison with reference images, first find the corresponding webpage interface and take a screenshot, then compare with the reference image.
3) For aesthetic type test tasks, you need to navigate to the corresponding webpage and name the screenshot according to “webpage_screenshot_path”.
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="WebCompass Image Evaluation Runner")
    ...
    # Get model list: prioritize command line argument, otherwise use model field in config file
    if args.models:
        model_list = [m.strip() for m in args.models.split(",") if m.strip()]
    elif config.get("model"):
        model_list = [config["model"]]
    else:
        print("Error: Please specify model via --models parameter or model field in config file")
        return
    ...
    for model_name in model_list:
        ...
        # existing_site_root 直接使用配置文件中的值，不再拼接 model_name
